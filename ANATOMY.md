...
**코드 규모**: ~232개 소스 파일, ~24,246줄 (Cominomi.Shared에 집중, tests/Cominomi.Shared.Tests 포함)
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Services/GitService.cs` | 708 | 모든 git 명령 (`IProcessRunner` 기반) + 설정 가능 git 경로 |
...
| **2** | **GitService 708줄 God Object** — cloning, branching, diff 파싱, 캐싱, stash, rebase + git 경로 해석을 단일 클래스에서 담당 | SRP 위반, 테스트 어려움 | 높 |
...
| **5** | **ParseDiff `[Obsolete]` 표시** — `GetDiffSummaryAsync` 사용 권장. 테스트에서만 호출. 향후 삭제 시 테스트 9개 마이그레이션 필요 | 코드 중복(경감됨) | 낮 |
