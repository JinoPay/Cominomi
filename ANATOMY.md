...
**코드 규모**: ~155개 소스 파일, ~16,250줄 (Cominomi.Shared에 집중, tests/Cominomi.Shared.Tests 포함)
...
| 새 파일 / 변경 내용 | 역할 / 영향 범위 |
|---------------------|-----------------|
| `tests/Cominomi.Shared.Tests/` (프로젝트 신규) | xUnit 테스트 인프라 — 53개 테스트 (ContentGrouper, QuestionDetector, ToolDisplayHelper, ExtractToolResultContent, SessionStatusMachine) |
...
- [구조적 문제 Top 10 (미해결)](#구조적-문제-top-10-영향도-순위--현재-미해결)
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `src/Cominomi/MauiProgram.cs` | 117 | DI 컨테이너 전체 설정 + Spotlight 복구 |
...
4. **서비스 등록** (`:63-92`): **30개 전부 Singleton**
...
- **ChatState에 인터페이스 없음** (`:64`): 직접 클래스로 등록되어 테스트 불가
...
- **인덱싱 없음**: `GetSessionsAsync()`가 모든 메타데이터 파일을 역직렬화 — O(n) 성능. 세션 수 증가 시 사이드바 로딩 느려짐
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Components/Chat/ChatView.razor` | ~899 | 오케스트레이터 (Phase 3에서 994→899줄 감소) |

### 현재 동작

**주입 서비스 14개** (Phase 3에서 18→14개로 축소):
```
ChatState, ClaudeService, SessionService, AttachmentService,
HooksEngine, JSRuntime, Logger, Snackbar,
UsageService, NotificationService,
SystemPromptBuilder, SessionInitializer, ChatPrWorkflowService  ← Phase 3 추가
```
> 제거됨: ContextService, MemoryService, IGhService, IGitService, IWorkspaceService, ISettingsService, ISessionGitWorkflowService
...
### 빠진 것 / 문제점
- ~~**God Component (1,461줄, 18개 서비스)**~~ → ✅ **해결**: Phase 1에서 `StreamEventProcessor`(516줄) 추출, Phase 3에서 `SystemPromptBuilder`, `SessionInitializer`, `ChatPrWorkflowService` 3개 서비스 추가 추출. **1,461→994→899줄, 18→14개 서비스**로 38% 감소. 추출된 서비스는 독립 테스트 가능.
...
| `Chat/` | 10 | ChatView, InputArea, MessageBubble, LinkIssueDialog, ActivitySummary, PlanReviewBar, QuickResponseBar, StreamingIndicator, **ModelSelector** (Phase 5), **AttachmentChips** (Phase 5) |
| `Diff/` | 1 | DiffPanel |
| `Files/` | 2 | FileContentView, FileDiffView |
| `Layout/` | 4 | MainLayout, DetailPanel, MainTabBar, StatusBar |
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Services/PluginService.cs` | 75 | 플러그인 탐색 + 매니페스트 파싱 |

### 현재 동작
`~/.claude/plugins/` 디렉토리 하위의 각 폴더에서 `manifest.json`을 읽어 플러그인 목록 획득. DI에 `IPluginService`로 등록.

### 빠진 것 / 문제점
- **플러그인 실행 미구현**: 매니페스트 파싱만 하고, 실제 플러그인 로딩/실행 메커니즘 없음
...
- **SnackbarExtensions 한국어 하드코딩**: 12개 알림 메서드 (Push/PR/Merge/Workspace/Settings/Session/Conflict/Streaming/Issue)의 텍스트가 모두 한국어 문자열. 로컬라이제이션 인프라 없음
...
- **로컬라이제이션 인프라 부재**: `.resx` 파일 0개, `IStringLocalizer` 미사용. `SnackbarExtensions`(12개 메서드), `ToolDisplayHelper`(14+ 문자열), `QuestionDetector`, `ContentGrouper` 등에 한국어 문자열 하드코딩. 다국어 지원 불가
...
```
ChatView.razor          ← 14개 서비스 (Phase 3에서 18→14개 축소)
...
### 빠진 것 / 문제점
- ~~**ChatView가 커플링 허브 (18개 서비스)**~~ → ✅ **부분 해결**: Phase 3에서 14개로 축소. 추출된 3개 서비스가 중간 계층 역할
...
- ~~**테스트 0개**: 전체 코드베이스에 단위/통합 테스트 없음~~ → ✅ **해결**: Phase 4에서 `tests/Cominomi.Shared.Tests/` 프로젝트 도입. 53개 테스트 (ContentGrouper 14, QuestionDetector 11, ToolDisplayHelper 13, ExtractToolResultContent 6, SessionStatusMachine 35). `InternalsVisibleTo`로 internal 메서드 테스트 가능
...
# 해결된 구조적 문제 (Phase 1–5)

| 문제 | 해결 방법 | Phase |
|------|-----------|-------|
| ChatView God Component (1,461줄, 18개 서비스) | `StreamEventProcessor` + `SystemPromptBuilder` + `SessionInitializer` + `ChatPrWorkflowService` 추출. 899줄, 14개 서비스 | 1, 3 |
...
| 테스트 0개 | `tests/Cominomi.Shared.Tests/` — 53개 테스트 | 4 |
...
> - "Top 1: 로컬라이제이션 인프라 추가 — `.resx` + `IStringLocalizer` 도입"
> - "Top 3: Session 모델 분리 — 대화 / Git / PR 관심사를 각각의 모델로"
> - "Top 5: ChatState에 `IChatState` 인터페이스 추가 + DI 등록 변경"
