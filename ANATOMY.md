...
- ~~**백업/마이그레이션 없음**: JSON 스키마 변경 시 기존 파일이 역직렬화 실패 → 데이터 손실~~ → ✅ **해결**: `JsonMigrator` 스키마 마이그레이션 엔진 도입. 모든 모델에 `schemaVersion` 필드 + 순차 마이그레이션 자동 적용 + `AtomicFileWriter`로 안전한 기록
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Components/Layout/MainLayout.razor` | ~150 | 3패널 레이아웃 셸 (테마/의존성 체크 위임, 리사이즈 상태 관리) |
...
| **5** | ~~**Git/Activity 캐싱 없음**~~ ✅ | `DetectDefaultBranchAsync`(5분), `ListBranches*`(30초), `GetWorkspaceActivityAsync`(10초) TTL 캐시 적용. fetch 시 자동 무효화 | §5, §19.5 | 중 |
...
| ~~2~~ | ~~패널 크기 조절 불가~~ | ✅ 해결 — PanelResizer 컴포넌트로 드래그 리사이즈 + 키보드 네비게이션 구현 | §11 | — |
| ~~**2**~~ | ~~**JSON 스키마 마이그레이션 없음**~~ | ~~스키마 변경 시 기존 파일 역직렬화 실패 → 데이터 손실. 버전 필드 없음~~ → ✅ **해결**: `JsonMigrator` + `MigrationRegistry` + 모델 `schemaVersion` 필드 | §2 | 높 |
| **~~3~~** | **~~워크트리 레이스 컨디션~~** | ~~Pending 세션에 빠르게 2번 전송 시 워크트리 이중 생성 시도 가능~~ ✅ 해결: per-session SemaphoreSlim + 캐시 무효화 + UI 가드 | §8 | 중 |
| ~~**4**~~ | ~~**활동 서비스 파이프 구분자 취약**~~ | NUL 구분자(`%x00`)로 변경 + Archived 세션 활동 조회 지원 | §19.5 | 낮 |
...
> - ~~"Top 8: SessionList에 `<Virtualize>` 컴포넌트 적용"~~ ✅ 해결됨
