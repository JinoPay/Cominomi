...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Services/PluginService.cs` | ~95 | 플러그인 탐색 + 매니페스트 파싱 + 로드/언로드 위임 |
| `Shared/Services/PluginExecutionEngine.cs` | ~260 | 플러그인 실행 엔진: 로드/언로드/실행 + hooks·skills 등록 |
...
| 순위 | 문제 | 영향 | 관련 섹션 | 난이도 |
|------|------|------|-----------|--------|
| ~~**1**~~ | ~~**플러그인 실행 엔진**~~ | EntryPoint 로딩/실행/샌드박싱 구현 완료. hooks·skills 매니페스트 등록 | §15.5 | — |
