...
**코드 규모**: ~180개 소스 파일, ~17,721줄 (Cominomi.Shared에 집중, tests/Cominomi.Shared.Tests 포함)
...
| #126 | 플러그인 시스템 인프라 | 매니페스트 파싱 + 상태 관리 (실행 엔진은 미구현) |
...
| `MainLayout.razor` 분해 | 238→~115줄(52%↓). 3패널 셸 + MudThemeProvider + 의존성 체크만 남음 |
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Components/Layout/MainLayout.razor` | ~115 | 3패널 레이아웃 셸 (테마/의존성 체크 위임) |
...
### 현재 동작
`~/.claude/plugins/` 디렉토리 하위의 각 폴더에서 `manifest.json`을 읽어 플러그인 목록 획득. DI에 `IPluginService`로 등록. `PluginStatus` enum으로 상태 관리 (Discovered, Valid, Invalid, Loaded, Error). 설정에서 enable/disable 가능.

### 빠진 것 / 문제점
- **플러그인 실행 엔진 미구현**: 매니페스트 파싱 + 상태 관리까지만 구현. EntryPoint 파일이 파싱되지만 실제 로딩/실행 메커니즘 없음. `PluginStatus.Loaded` 값은 사용되지 않음
...
- ~~**테스트 0개**: 전체 코드베이스에 단위/통합 테스트 없음~~ → ✅ **해결**: Phase 4+에서 `tests/Cominomi.Shared.Tests/` 프로젝트 도입. 84개 테스트 (8개 테스트 파일). `InternalsVisibleTo`로 internal 메서드 테스트 가능
...
| ~~**9**~~ | ~~플러그인 시스템 미구현~~ | 매니페스트 파싱 + 상태 관리 + enable/disable. **단, 실행 엔진은 미구현** (인프라만 완성) | #126 |
...
| ~~**3**~~ | ~~**MainLayout 과다 책임**~~ | `IThemeService` 추출 + `SidebarToolbar`/`MainToolbar` 컴포넌트 분리. 238→~115줄(52%↓) | Phase 7 |
| ~~**4**~~ | ~~**macOS 알림 미구현**~~ | ~~MacCatalyst 타겟이지만 NotificationService는 Windows 전용~~ | §21 | ✅ 해결 |
| ~~5~~ | ~~**옵션 패턴 미사용**~~ | ~~각 서비스가 `ISettingsService.LoadAsync()` 직접 호출~~ → ✅ **해결**: `IOptionsMonitor<AppSettings>` 도입 | §1 | ✅ 해결 |
