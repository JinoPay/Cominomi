...
| ~~**10**~~ | ~~**플러그인 stdin/stdout 프로토콜 미정의** — 환경변수만 전달~~ | `PluginExecutionEngine.cs` — JSON stdin/stdout 프로토콜(`json/1`) 도입. `PluginRequest`→stdin, stdout→`PluginResponse` 구조화 데이터 교환. 환경변수 하위호환 유지 | §15.5 | 높 |
