...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `src/Cominomi/MauiProgram.cs` | 100 | DI 컨테이너 전체 설정 |
...
### 현재 동작
`MauiProgram.cs:14-98`에서 앱이 부트스트랩됨:
...
4. **서비스 등록** (`:63-88`): **26개 전부 Singleton**

```
등록 순서 (MauiProgram.cs:63-88):
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Services/SessionService.cs` | 413 | 세션 CRUD + 워크트리 |
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Components/Chat/ChatView.razor` | 994 | **God Component** (StreamEventProcessor 추출 후) |
...
### 빠진 것 / 문제점
- **God Component**: ~~1,461줄에 18개 서비스~~ → ✅ **부분 해결**: `StreamEventProcessor`(516줄) 서비스로 스트림 이벤트 처리, 사용량 기록 (3단계 폴백), 플랜 감지 (3계층)를 추출. **현재 994줄, 18개 서비스**. 여전히 프로젝트 최대 컴포넌트.
...
| `Chat/` | 8 | ChatView, InputArea, MessageBubble, LinkIssueDialog, ActivitySummary, PlanReviewBar, QuickResponseBar, StreamingIndicator |
| `Diff/` | 1 | DiffPanel |
...
| `Setup/` | 1 | SetupDialog |
| `Sidebar/` | 4 | SessionList, CreateWorkspaceDialog, SidebarExplorer, SidebarChanges |
...
### `async void` 안티패턴 (6개 컴포넌트)

`ChatState.OnChange`에 등록되는 이벤트 핸들러가 `async void`로 선언:

| 컴포넌트 | 메서드 |
|----------|--------|
| `ChatView.razor` | `async void HandleStateChanged()` |
| `DetailPanel.razor` | `async void HandleStateChanged()` |
| `MainLayout.razor` | `async void HandleSettingsChanged()` |
| `SessionList.razor` | `async void OnStateChanged()` |
| `SidebarChanges.razor` | `async void HandleStateChanged()` |
| `SidebarExplorer.razor` | `async void HandleStateChanged()` |

**위험**: `async void`의 예외는 호출자에게 전파되지 않고 `TaskScheduler.UnobservedTaskException`으로만 관찰 가능. Blazor 에러 경계를 우회하여 UI가 무반응 상태에 빠질 수 있음.
...
- **async void 예외 미관찰**: 위 표 참조. 6개 컴포넌트에서 반복
...
| 순위 | 문제 | 영향 | 관련 파일 | 상태 |
|------|------|------|-----------|------|
| **1** | ChatView God Component (~994줄, 18개 서비스) | 모든 기능 변경이 하나의 파일에 집중 | `ChatView.razor` | ✅ **부분 해결** — `StreamEventProcessor`(516줄)로 스트림/사용량/플랜 추출. 1,461→994줄로 32% 감소. 여전히 프로젝트 최대 컴포넌트 |
| **2** | JSON 영속화에 파일 락 없음 | 병렬 세션의 동시 저장 시 데이터 손상 | `SessionService.cs` | ✅ **해결** — `AtomicFileWriter` + per-session `SemaphoreSlim` 락 |
...
| **6** | ~~프로세스 실행 복붙~~ | ~~타임아웃/에러처리 불일치~~ | | ✅ **해결** — 7개 서비스 `IProcessRunner` 마이그레이션 완료 |
...
| **9** | ~~가격/모델/CLI플래그 하드코딩~~ | ~~외부 변경마다 코드 수정 필요~~ | | ✅ **해결** — `ModelDefinitions` + `models.json` + `CominomiConstants` |
| **10** | 테스트 0개 | 리팩터링 위험, 회귀 감지 불가 | 전체 | 미해결 |
| **신규** | `async void` 안티패턴 (6개 컴포넌트) | 예외 미관찰, UI 무반응 가능 | ChatView, DetailPanel, MainLayout, SessionList, SidebarChanges, SidebarExplorer | 미해결 |
| **신규** | 로컬라이제이션 인프라 부재 | 한국어 하드코딩, 다국어 지원 불가 | SnackbarExtensions, ToolDisplayHelper, QuestionDetector, ContentGrouper | 미해결 |
