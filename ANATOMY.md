...
### 구조 개선 Phase 12 (2026-03-19) — 차기 개선 후보 #5 해결 (스킬 체이닝)
| 변경 내용 | 역할 / 영향 범위 |
|-----------|-----------------|
...
- ~~**스킬 체이닝 불가**~~: Phase 12에서 해결. `/commit | /review` 파이프 구문 + 커스텀 스킬 `chain:` 프론트매터 지원
...
| ~~**5**~~ | ~~**스킬 체이닝 불가**~~ — Phase 12에서 해결. 파이프 구문 `/commit \| /review` + 커스텀 스킬 `chain:` 프론트매터 | ~~생산성~~ | §15 | ~~중~~ |
