...
| **4** | **ContentGrouper 중간 텍스트 휴리스틱** — 하드코딩된 한국어/영어 패턴으로 텍스트 분류. 다국어 확장 시 패턴 폭발 | 렌더링 오분류 | §13 | 낮 |
| **5** | **ToolCallCard JSON 매 렌더 파싱** — 도구 입력 JSON을 캐싱 없이 매 렌더마다 `JsonSerializer.Deserialize` | 렌더 성능 | §13 | 낮 |
| **6** | **FileSystemWatcher 이벤트 폭주** — Windows에서 빠른 파일 변경 시 이벤트 과다 → UI 업데이트 과부하 | UI 버벅임 | §12 | 중 |
| **7** | **알림 히스토리 없음** — 놓친 알림을 확인할 방법 없음 | UX | §21 | 중 |
| **8** | **MCP 서버 수정 불가** — 기존 서버 설정 변경이 불가. 삭제 후 재추가만 가능 | UX | §16 | 낮 |
| **9** | **스킬 체이닝 불가** — `/commit` 후 `/review` 같은 워크플로우 자동화 미지원 | 생산성 | §15 | 중 |
