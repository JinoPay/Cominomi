    ...
    ILogger<ClaudeAccountService> logger) : IClaudeAccountService
{
    ...
    public async Task<AccountExportEnvelope> PrepareExportAsync(string accountId)
    {
        await _lock.WaitAsync();
        try
        {
            ...
            var backup = await credentialService.LoadBackupAsync(accountId)
                ?? throw new InvalidOperationException("백업 파일이 없어 내보낼 수 없습니다. 계정을 다시 등록하세요.");

            // Normalize credentials to flat format (accessToken / refreshToken)
            var accessToken = ExtractToken(backup.CredentialsJson ?? "", "accessToken", "access_token");
            var refreshToken = ExtractToken(backup.CredentialsJson ?? "", "refreshToken", "refresh_token");

            // Parse config as JsonObject (may be null if not present)
            JsonObject? configObj = null;
            if (backup.ConfigJson != null)
            {
                try { configObj = JsonNode.Parse(backup.ConfigJson)?.AsObject(); } catch { /* ignore */ }
            }

            return new AccountExportEnvelope
            {
                SourcePlatform = OperatingSystem.IsWindows() ? "win32" : "darwin",
                Account = new AccountExportData
                {
                    ProfileName = account.ProfileName,
                    EmailAddress = account.EmailAddress,
                    AccountUuid = account.AccountUuid,
                    CreatedAt = account.CreatedAt,
                    SwitchCount = account.SwitchCount,
                    TotalActiveSeconds = account.TotalActiveSeconds,
                },
                Config = configObj,
                Credentials = new AccountExportCredentials
                {
    ...
    public async Task<ClaudeAccount> ImportAccountAsync(AccountExportEnvelope envelope, string profileName)
    {
        await _lock.WaitAsync();
        try
        {
            ...
            // Reconstruct platform-appropriate credentials JSON
            string credentialsJson;
            var at = envelope.Credentials.AccessToken ?? "";
            var rt = envelope.Credentials.RefreshToken ?? "";
            ...
            // Reconstruct config JSON — use the exported config if available
            string configJson;
            if (envelope.Config != null)
            {
                configJson = envelope.Config.ToJsonString(new JsonSerializerOptions { WriteIndented = true });
            }
            else
            {
                configJson = $$"""
                    {
                      "oauthAccount": {
                        "emailAddress": "{{envelope.Account.EmailAddress}}",
                        "accountUuid": "{{envelope.Account.AccountUuid}}"
                      }
                    }
                    """;
            }

            var newAccount = new ClaudeAccount
            {
                ProfileName = profileName,
                EmailAddress = envelope.Account.EmailAddress,
                AccountUuid = envelope.Account.AccountUuid,
                CreatedAt = envelope.Account.CreatedAt,
                SwitchCount = envelope.Account.SwitchCount,
                TotalActiveSeconds = envelope.Account.TotalActiveSeconds,
                IsActive = false,
            };

            await credentialService.BackupAsync(newAccount.Id, configJson, credentialsJson);

            store.Accounts.Add(newAccount);
            await SaveStoreAsync(store);

            logger.LogInformation("Imported account {Email} as '{Profile}' ({Id})",
                newAccount.EmailAddress, profileName, newAccount.Id);
